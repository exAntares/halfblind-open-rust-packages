...
pub struct TransactionResult {
    ...
    pub rewarded: Vec<InventoryItem>,
}

pub fn has_enough_item_definitions(
...
pub fn has_any_item_definitions(
    inventory: &Vec<InventoryItem>,
...
pub fn get_instant_rewards_items_into_inventory(
    player_uuid: Uuid,
    inventory: &mut Vec<InventoryItem>,
...
pub async fn process_player_transaction(
    inventory_service: Arc<dyn InventoryService + Send + Sync>,
    ...
    { // Acquire read lock on inventory
        ...
        if !has_enough_item_definitions(&player_inventory, &transaction.required) {
            return Err(ItemsErrorCode::TransactionRequirementsNotMet.into());
        }

        if has_any_item_definitions(&player_inventory, &transaction.required_negative) {
            return Err(ItemsErrorCode::TransactionRequirementsNotMet.into());
        }

        if !has_enough_item_definitions(&player_inventory, &transaction.consumed) {
